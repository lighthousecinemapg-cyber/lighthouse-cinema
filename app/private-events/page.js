...
const AUDITORIUM_RATES = {
  // Rate A: Small Nonprofits (<$5M revenue)
  smallNonprofit: {
  ...
  // Rate B: Large Nonprofits & Individuals
  largeNonprofit: {
  ...
  // Rate C: Corporations / Large Businesses
  corporate: {
...
const EVENT_TYPES = [
  ...
  { id: 'movie', icon: '🎬', label: 'Private Movie Screening' },
...
/* ───────────────────────────────────────────
   HELPER: determine day vs evening
   Auditorium: Day = 9am-6pm Mon-Wed, 9am-5pm Thu-Sun
   Mezzanine:  Day = 8am-7pm Mon-Sat, Sunday all day
   ─────────────────────────────────────────── */
function getTimeOfDay(hour, dayOfWeek, space) {
  ...
  // Auditorium
  if (dayOfWeek === 0) return hour < 17 ? 'day' : 'evening'; // Sunday
  if (dayOfWeek >= 4) return hour < 17 ? 'day' : 'evening'; // Thu-Sat
  return hour < 18 ? 'day' : 'evening'; // Mon-Wed
}
...
export default function PrivateEventsPage() {
  ...
  const dayOfWeek = selectedDate ? selectedDate.getDay() : 5; // default Friday
  ...
  const hourlyRate =
    space && rateType
      ? getHourlyRate(space, rateType, dayOfWeek, timeOfDay)
      : 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      const booking = {
        ...
        addonIds: [
          ...(addTech ? ['tech-support'] : []),
          ...(addMics ? ['extra-mics'] : []),
          ...(addCustomSetup ? ['custom-setup'] : []),
        ],
        ...
        <div style={styles.container}>
          <div style={{ textAlign: 'center', padding: '80px 20px' }}>
            ...
            <p style={styles.subText}>
              Thank you, {name}. We&apos;ll confirm your {EVENT_TYPES.find(e => e.id === eventType)?.label || 'event'} within 24 hours.
            </p>
            <p style={{ ...styles.subText, marginTop: 10, opacity: 0.6 }}>
              A confirmation has been sent to {email}
            </p>
            <div style={{ ...styles.priceBox, marginTop: 40, maxWidth: 360, marginLeft: 'auto', marginRight: 'auto' }}>
              <div style={styles.priceRow}>
                <span>Estimated Total</span>
                <span style={styles.goldText}>${total.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
              </div>
              <div style={{ ...styles.priceRow, opacity: 0.6, fontSize: 13 }}>
                <span>+ $500 refundable security deposit</span>
              </div>
            </div>
            <a href="/" style={{ ...styles.btn, display: 'inline-block', marginTop: 40 }}>
              Back to Home
            </a>
          </div>
        </div>
  ...
  return (
    <div style={styles.page}>
      ...
      <section style={styles.hero}>
        <div style={styles.heroOverlay}>
          ...
          <h1 style={styles.heroTitle}>
            Host Your Event Inside<br />a <span style={styles.goldText}>Real Cinema</span>
          </h1>
          <p style={styles.heroSub}>Private. Cinematic. Unforgettable.</p>
          <a href="#booking" style={styles.heroCta}>
            Check Availability &amp; Price Instantly
          </a>
        </div>
      </section>
      ...
      <div style={styles.trustBar}>
        ...
        <span>🎤 Full AV System</span>
        <span>🍿 In-House Catering</span>
      </div>

      {/* BOOKING WIZARD */}
      <section id="booking" style={styles.container}>
        <h2 style={{ ...styles.sectionTitle, textAlign: 'center', marginBottom: 10 }}>
          Book Your Private Event
        </h2>
        <p style={{ textAlign: 'center', color: '#999', marginBottom: 40, fontSize: 15 }}>
          Get an instant price estimate in under 60 seconds
        </p>
        ...
        <div style={styles.progressWrap}>
          {[1, 2, 3, 4, 5, 6, 7].map((s) => (
            <div
              ...
              style={{
                ...styles.progressDot,
                background: step >= s ? '#D4AF37' : '#333',
                transform: step === s ? 'scale(1.3)' : 'scale(1)',
              }}
            />
          ))}
          <div style={{ ...styles.progressLine, width: `${((step - 1) / 6) * 100}%` }} />
        </div>
        ...
        <div style={styles.wizardCard}>
              ...
              <div style={styles.optionGrid}>
                {EVENT_TYPES.map((et) => (
                  <button
                    ...
                    style={{
                      ...styles.optionCard,
                      borderColor: eventType === et.id ? '#D4AF37' : '#333',
                      background: eventType === et.id ? 'rgba(212,175,55,0.08)' : '#1a1a1a',
                    }}
                  >
          ...
          {/* STEP 2: Space + Rate Type */}
          {step === 2 && (
            <div>
              <h3 style={styles.stepTitle}>Choose your space</h3>
              <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', marginBottom: 30 }}>
                <button
                  onClick={() => setSpace('auditorium')}
                  style={{
                    ...styles.spaceCard,
                    borderColor: space === 'auditorium' ? '#D4AF37' : '#333',
                    background: space === 'auditorium' ? 'rgba(212,175,55,0.08)' : '#1a1a1a',
                  }}
                >
                ...
                <button
                  onClick={() => setSpace('mezzanine')}
                  style={{
                    ...styles.spaceCard,
                    borderColor: space === 'mezzanine' ? '#D4AF37' : '#333',
                    background: space === 'mezzanine' ? 'rgba(212,175,55,0.08)' : '#1a1a1a',
                  }}
                >
                  <span style={{ fontSize: 40 }}>🥂</span>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {Object.entries(space === 'mezzanine' ? MEZZANINE_RATES : AUDITORIUM_RATES).map(([key, val]) => (
                  <button
                    ...
                    style={{
                      ...styles.rateOption,
                      borderColor: rateType === key ? '#D4AF37' : '#333',
                      background: rateType === key ? 'rgba(212,175,55,0.08)' : '#1a1a1a',
                    }}
                  >
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap' }}>
                <div style={{ flex: 1, minWidth: 200 }}>
                  ...
                  />
                  {dayName && (
                    <p style={{ color: '#D4AF37', fontSize: 14, marginTop: 8 }}>
                      {dayName} — {timeOfDay === 'sunday' ? 'Sunday' : timeOfDay.charAt(0).toUpperCase() + timeOfDay.slice(1)} rate
                    </p>
                  )}
                </div>
                <div style={{ flex: 1, minWidth: 200 }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 20, justifyContent: 'center' }}>
                ...
                <div style={{ textAlign: 'center' }}>
                  ...
                  <span style={{ display: 'block', fontSize: 14, color: '#999' }}>
              ...
              <input
                ...
                style={{ width: '100%', marginTop: 20, accentColor: '#D4AF37' }}
              />
            ...
            <div>
              ...
              <div style={{ textAlign: 'center', marginTop: 20 }}>
                <input
                  type="number"
                  min="1"
                  max={space === 'mezzanine' ? 80 : 145}
                  value={guests}
                  onChange={(e) => setGuests(Math.max(1, Number(e.target.value)))}
                  style={{ ...styles.input, textAlign: 'center', fontSize: 32, maxWidth: 150, margin: '0 auto' }}
                />
                <p style={{ color: '#888', fontSize: 13, marginTop: 10 }}>
                  {space === 'auditorium'
                    ? 'Auditorium seats up to 145 guests'
                    : 'Mezzanine accommodates intimate groups'}
                </p>
              </div>
            </div>
          )}
          ...
          {step === 6 && (
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 10 }}>
                <label style={{
                  ...styles.addonCard,
                  borderColor: addTech ? '#D4AF37' : '#333',
                  background: addTech ? 'rgba(212,175,55,0.08)' : '#1a1a1a',
                }}>
                  <input
                    type="checkbox"
                    checked={addTech}
                    onChange={(e) => setAddTech(e.target.checked)}
                    style={{ accentColor: '#D4AF37', width: 18, height: 18 }}
                  />
                  <div style={{ flex: 1 }}>
                    <strong>🎛️ Tech Support</strong>
                    <span style={{ display: 'block', fontSize: 12, color: '#999' }}>
                      Dedicated technician — $100/hr
                    </span>
                  </div>
                  {addTech && (
                    <select
                      ...
                      style={{ ...styles.input, width: 70, padding: '6px 8px', fontSize: 13 }}
                    >
                      {[1, 2, 3, 4, 5, 6].map((h) => (
                ...
                <label style={{
                  ...styles.addonCard,
                  borderColor: addMics ? '#D4AF37' : '#333',
                  background: addMics ? 'rgba(212,175,55,0.08)' : '#1a1a1a',
                }}>
                  <input
                    type="checkbox"
                    checked={addMics}
                    onChange={(e) => setAddMics(e.target.checked)}
                    style={{ accentColor: '#D4AF37', width: 18, height: 18 }}
                  />
                  <div style={{ flex: 1 }}>
                    <strong>🎤 Extra Microphones</strong>
                    <span style={{ display: 'block', fontSize: 12, color: '#999' }}>
                      Additional wireless mics (included: 1-2 standard)
                    </span>
                  </div>
                </label>
                <label style={{
                  ...styles.addonCard,
                  borderColor: addCustomSetup ? '#D4AF37' : '#333',
                  background: addCustomSetup ? 'rgba(212,175,55,0.08)' : '#1a1a1a',
                }}>
                  <input
                    type="checkbox"
                    checked={addCustomSetup}
                    onChange={(e) => setAddCustomSetup(e.target.checked)}
                    style={{ accentColor: '#D4AF37', width: 18, height: 18 }}
                  />
                  <div style={{ flex: 1 }}>
                    <strong>✨ Custom Setup / Decor</strong>
                    <span style={{ display: 'block', fontSize: 12, color: '#999' }}>
                      Custom arrangement, signage, or decorations
                    </span>
                  </div>
                </label>
              </div>
          ...
          {step === 7 && (
            <div>
              <h3 style={styles.stepTitle}>Almost there — your details</h3>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 30, gap: 12 }}>
            {step > 1 ? (
              <button onClick={() => setStep(step - 1)} style={styles.btnOutline}>
                ← Back
              </button>
            ) : <div />}
            {step < 7 ? (
              ...
              <button
                ...
                disabled={!canProceed(7) || submitting}
                style={{
                  ...styles.btn,
                  opacity: canProceed(7) && !submitting ? 1 : 0.4,
                  cursor: canProceed(7) && !submitting ? 'pointer' : 'not-allowed',
                  fontSize: 16,
                  padding: '14px 32px',
                }}
              >
        ...
        {/* LIVE PRICE CALCULATOR (sticky on desktop) */}
        {step >= 2 && space && (
          <div style={styles.priceBox}>
            <h4 style={{ color: '#D4AF37', margin: '0 0 16px', fontSize: 16, letterSpacing: 1 }}>
              YOUR ESTIMATE
            </h4>
            <div style={styles.priceRow}>
              <span>{space === 'auditorium' ? 'Auditorium' : 'Mezzanine'} — {duration}hr × ${hourlyRate}</span>
            ...
            {addTech && (
              <div style={styles.priceRow}>
                <span>Tech support — {techHours}hr × $100</span>
                <span>${techCost.toLocaleString()}</span>
              </div>
            )}
            <div style={{ ...styles.priceRow, borderTop: '1px solid #333', paddingTop: 10, marginTop: 6 }}>
            ...
            <div style={{ ...styles.priceRow, borderTop: '2px solid #D4AF37', paddingTop: 12, marginTop: 8 }}>
              <strong style={{ fontSize: 18 }}>TOTAL</strong>
              <strong style={{ fontSize: 24, color: '#D4AF37' }}>
                ${total.toLocaleString('en-US', { minimumFractionDigits: 2 })}
              </strong>
            </div>
            <p style={{ fontSize: 11, color: '#777', marginTop: 12, lineHeight: 1.5 }}>
              + $500 refundable security deposit<br />
      ...
      <section style={{ maxWidth: 800, margin: '40px auto', padding: '0 20px' }}>
        <div style={{ background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: 12, padding: 24 }}>
          ...
          <div style={{ fontSize: 13, color: '#999', lineHeight: 1.8 }}>
            <p>🚫 No outside food or drink (cakes permitted with written approval)</p>
            <p>💳 Full payment required at time of contract signing</p>
            <p>🔒 $500 refundable security deposit (returned within 3 weeks)</p>
            <p>📜 Renter is responsible for all applicable licensing fees</p>
            <p>⏰ Rental time includes setup and teardown</p>
            <p>🎬 Basic projection and PA system with 1-2 microphones included</p>
            <p>📋 One complimentary site visit or content testing session (30 min)</p>
          </div>
        </div>
      </section>
      ...
      <style>{`
        ...
        #booking .wizardCard > div { animation: fadeIn 0.35s ease-out; }
        input[type="range"]::-webkit-slider-thumb { cursor: grab; }
        input[type="date"]::-webkit-calendar-picker-indicator { filter: invert(1); }
        @media (max-width: 768px) {
          .trust-bar-inner { flex-direction: column !important; gap: 8px !important; }
        }
      `}</style>
    </div>
  );
